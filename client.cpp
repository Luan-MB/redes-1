        ...
        if (opt == 'a') {
            ...
            if (!arq) {
                ...
                if (sb.st_size <= UINT32_MAX) {
                    ...
                    char *media_buffer = (char *) malloc (file_name.length() + 4);
                    memcpy(media_buffer, &(sb.st_size), 4);
                    memcpy(&media_buffer[4], file_name.c_str(), file_name.length());

                    Mensagem media{Midia, seq, (unsigned char) (file_name.length() + 4), media_buffer};
                        ...
                        if ((retval = Controller::recvAck(socket, recv_buffer)) == 20) {
                            ...
                            if ((response->tipo == Ack) && (response->sequencia == seq)) {
                                ...
                                switch (response->dados[0]) {
                                    case 0x65:
                                        std::cerr << "\033[31mFalha ao enviar o arquivo: " << file_name 
                                        << ". Nao ha espaco suficiente para armazena-lo!\033[0m" << std::endl;
                                        break;
                                    case 0x66:
                    ...
                    std::cerr << "\033[31mFalha ao enviar o arquivo: " << file_path << ". Arquivo muito grande!\033[0m" << std::endl;
                }
            }
